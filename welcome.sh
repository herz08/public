...
if ( ifconfig | grep -q "eth0" ) ; then IP_LAN=`ifconfig eth0 | grep "inet " | awk '{print $2}'` ; else IP_LAN="---" ; fi ;
if ( ifconfig | grep -q "wlan0" ) ; then IP_WLAN=`ifconfig wlan0 | grep "inet " | awk '{print $2}'` ; else IP_WLAN="---" ; fi ;
if ( ifconfig | grep -q "eth0" ) ; then IP6_LAN=`ifconfig eth0 | grep "inet6 " | awk '{print $2}'` ; else IP6_LAN="---" ; fi ;
if ( ifconfig | grep -q "wlan0" ) ; then IP6_WLAN=`ifconfig wlan0 | grep "inet6 " | awk '{print $2}'` ; else IP6_WLAN="---" ; fi ;
...
\033[1;31m   '~ .~~~. ~'    \033[0;37mIP4-Adressen..: LAN: \033[1;35m$IP_LAN\033[0;37m | WiFi: \033[1;35m$IP_WLAN
\033[1;31m       '~'        \033[0;37mIP6-Adressen..: LAN: \033[1;35m$IP6_LAN\033[0;37m | WiFi: \033[1;35m$IP6_WLAN
\033[m"
